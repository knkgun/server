...
class Manager implements ICommentsManager {
	...
	public function delete($id) {
		...
		if ($affectedRows > 0 && $comment instanceof IComment) {
			if ($comment->getVerb() === 'reaction_delete') {
	 ...
	 * @since 24.0.0
	 */
	public function retrieveCommentsOfReaction(int $parentId, string $reaction): ?array {
		$qb = $this->dbConn->getQueryBuilder();
		$result = $qb
			->select('message_id')
			->from('reactions')
		...
		$totalQuery
			->selectAlias(
				$totalQuery->func()->concat(
					...
					new Literal('reaction'),
					$totalQuery->expr()->literal('":'),
					new Literal($totalQuery->func()->count('id'))
				),
				'total'
			)
			->from('reactions', 'r')
