...
class Version24000Date20211222112246 extends SimpleMigrationStep {
	...
	public function changeSchema(IOutput $output, Closure $schemaClosure, array $options): ?ISchemaWrapper {
		...
		if (!$comments->hasColumn('reactions')) {
			...
			$table->addIndex(['reaction'], self::TABLE_NAME . '_reaction_index');
			$table->addIndex(['parent_id'], self::TABLE_NAME . '_parent_id_index');
			$table->addUniqueIndex(['parent_id', 'actor_type', 'actor_id', 'reaction'], self::TABLE_NAME . '_unique_index');
			return $schema;
		}
		return null;
	}
